...
class Args:
    ...
    def params(self, env):  #environment specific parameters calculation
        ...
        agent = env.agents[0]
        self.observations_dim = np.prod(env.observation_space(agent).shape)
...
class QMixer(nn.Module):
    ...
    def vectorize_obs(self, obs):
        return obs.reshape(self.args.observations_dim,1,1).squeeze(-1).squeeze(-1)
...
class AgentRNN(nn.Module):
    def __init__(self,args):
        ...
        self.net = nn.Sequential(
                nn.Linear(args.observations_dim, args.dim_L1_agents_net),
                nn.ELU(),
...
class runner_QMix:
    ...
    def run(self):
        ...
        for agent in self.env.agents:
            ...
            observation_prev[agent] = self.online_net.vectorize_obs(observation_prev[agent])
        for _ in range(args.MIN_BUFFER_LENGTH):
            ...
            for agent in self.env.agent_iter(max_iter=len(self.env.agents)):
                ...
                observation[agent] = self.online_net.vectorize_obs(observation[agent])
...
def init_env(environment):
    if environment == 'pistonball':
        env = pistonball_v6.env(continuous=False)  #simple_spread_v2.env(N=3, local_ratio=0.18, max_cycles=25, continuous_actions=False)
        env.reset()

        #taking only blue channel of observations
        env = ss.color_reduction_v0(env, mode='B')
        #shrinking the images
        env = ss.resize_v0(env, x_size=84, y_size=84)
        #stacking frames together to be able to take speed and acceleration into account
        env = ss.frame_stack_v1(env, 4)

        env.reset()
        return env
    elif environment == 'simple spread':
        simple_spread_v2.env(N=3, local_ratio=0.18, max_cycles=25, continuous_actions=False)
        env.reset()
        return env

if __name__ == "__main__":
    env = init_env('pistonball')
    args = Args(env)
