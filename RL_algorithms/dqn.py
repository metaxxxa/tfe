...
optimizer = torch.optim.Adam(online_net.net.parameters(), lr = LEARNING_RATE)
...
for step in itertools.count():
    ...
    q_values_online = torch.gather(input=online_net.get_Q_values(obses_t).squeeze(0), dim=1, index=actions_t)
    ...
    if np.mean(rew_buffer) > 185:
                
        while VISUALIZE_WHEN_LEARNED:
            obs = env.reset()
            env.render()
            obs, _, done, _ = env.step(online_net.act(obs))
            if done:
                obs = env.reset()
writer.close() 
