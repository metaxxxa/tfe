...
class QMixer(nn.Module):
    ...
    def get_Q_max(self, q_values): #gives out max q value index and max q value from array of q values
        max_q_index = torch.argmax(q_values, dim=1)[0].detach().item()
        return max_q_index, q_values[0,max_q_index]
...
########## checkpoint
    ...
    Q_ins_online_t.requires_grad_()
    Q_ins_target_t.requires_grad_()
    ...
    dones_t = dones_t > 0
    # targets
    Qtot_max_target = target_net.forward(Q_ins_target_t) #.max(dim=1, keepdim=True)[0]
    Qtot_online = online_net.forward(Q_ins_online_t)
